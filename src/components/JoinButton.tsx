...
type JoinState = 'idle' | 'loading' | 'joined' | 'pending' | 'full' | 'expired'
...
export function JoinButton({ state, approvalMode }: { state: JoinState; approvalMode?: boolean }) {
  ...
  if (state === 'expired') return <button disabled className={`${base} bg-red-100 text-red-700 cursor-not-allowed`}><AlertCircle className="h-4 w-4" />Expired</button>
