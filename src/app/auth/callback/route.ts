...
export async function GET(request: Request) {
  ...
  if (user) {
    const payload = {
      id: user.id,
      name:
        user.user_metadata?.name || user.email?.split("@")[0] || "Zuno User",
      avatar_url: null,
    ...
    if (
      ...
      error?.message?.includes("gpay_link") ||
      error?.message?.includes("phone_number") ||
      error?.message?.includes("avatar_seed") ||
      error?.message?.includes("upi_payee_name")
    ) {
      const {
        instagram_url,
        gpay_link,
