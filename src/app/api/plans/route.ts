...
export async function GET(request: Request) {
  ...
  if (includeMine && auth.user) {
    ...
    scopedPlanIds = Array.from(
      new Set(
        (joinedRows || []).map((row: any) => row.plan_id).filter(Boolean),
      ),
    );
  }
  ...
  let query = supabase
    ...
    .select(
      `
    *,
    host:users!plans_host_id_fkey(*),
    participants:plan_participants(
      user_id,
      status,
      user:users!plan_participants_user_id_fkey(
        id,
        name,
        avatar_url,
        gender
  ...
  if (!includeMine) {
    ...
    const filters = [`visibility.eq.public`, `host_id.eq.${auth.user.id}`];

    if (scopedPlanIds.length > 0) {
      filters.push(`id.in.(${scopedPlanIds.join(",")})`);
    }
    ...
    .map((p: any) => {
      ...
      return {
        ...
        is_joined:
          !!auth.user &&
          (p.participants || []).some(
            (pp: any) => pp.user_id === auth.user.id && pp.status === "joined",
          ),
  ...
  if (!auth.user) {
    return NextResponse.json(
      normalized.map((p: any) => ({ ...p, is_favorite: false })),
    );
  }
  ...
  return NextResponse.json(
    normalized.map((p: any) => ({ ...p, is_favorite: favSet.has(p.id) })),
  );
}
...
export async function POST(request: Request) {
  ...
  const visibility =
    body.visibility === "invite_only" || body.visibility === "private"
      ? "invite_only"
      : "public";
  ...
  const payload = {
    ...
    host_included_in_spots_and_splits:
      body.host_included_in_spots_and_splits !== false,
    status: "active",
  };
  ...
  if (
    error &&
    /(cost_mode|cost_amount|final_amount|visibility|host_mode)/.test(
      error.message,
    )
  ) {
    const { cost_mode, cost_amount, final_amount, ...fallbackPayload } =
      payload;
