...
const REPORT_REASONS = ["fake_profile", "harassment", "unsafe_plan", "spam", "other"];
...
export default function ProfilePage() {
  ...
  const loadProfile = async () => {
    ...
    if (!profile.avatar_seed) {
      ...
      if (isOwnProfile) await supabase.from("users").update({ avatar_seed: nextSeed }).eq("id", targetId);
    }
  ...
  return (
    <div className="bg-[#faf8f4] min-h-screen pb-24">
      <div className="sticky top-0 z-20 bg-[#faf8f4]/90 backdrop-blur border-b border-black/5 px-4 py-3 flex items-center justify-between">
        <button onClick={() => router.back()} className="p-2 rounded-full hover:bg-black/5">
      ...
      <div className="max-w-md mx-auto px-4 py-6 space-y-4">
        <div className="rounded-2xl bg-[#EFE7DA] p-5 text-center">
          <div className="relative mx-auto h-24 w-24 mb-3">
            <img src={displayedAvatar} className="h-full w-full rounded-full object-cover" />
            {isOwnProfile && (
              <button onClick={handleAvatarRegenerate} className="absolute bottom-0 right-0 h-8 w-8 rounded-full bg-black text-white flex items-center justify-center">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={item.label} className="rounded-xl bg-[#EFE7DA] p-3 text-center">
        ...
        {isOwnProfile && (
          <div className="rounded-2xl bg-white p-4 space-y-3 border border-black/5">
            <p className="text-sm font-medium flex items-center gap-1"><Sparkles className="h-4 w-4" /> Profile details</p>
            <input value={edit.name} onChange={(e) => setEdit({ ...edit, name: e.target.value })} placeholder="Name" className="w-full bg-[#f6f2ec] rounded-xl px-3 py-2 text-sm" />
            <input value={edit.avatarUrl} placeholder="Avatar url" onChange={(e) => setEdit({ ...edit, avatarUrl: e.target.value })} className="w-full bg-[#f6f2ec] rounded-xl px-3 py-2 text-sm" />
            <input value={edit.instagramUrl} placeholder="Instagram Url" onChange={(e) => setEdit({ ...edit, instagramUrl: e.target.value })} className="w-full bg-[#f6f2ec] rounded-xl px-3 py-2 text-sm" />
            <input value={edit.email} placeholder="Email Id" readOnly className="w-full bg-[#f6f2ec] rounded-xl px-3 py-2 text-sm text-gray-400" />
            ...
            <input value={edit.gpayLink} placeholder="UPI ID" onChange={(e) => setEdit({ ...edit, gpayLink: e.target.value })} className="w-full bg-[#f6f2ec] rounded-xl px-3 py-2 text-sm" />
            <input placeholder="Payee Name" value={edit.upiPayeeName} onChange={(e) => setEdit({ ...edit, upiPayeeName: e.target.value })} className="w-full bg-[#f6f2ec] rounded-xl px-3 py-2 text-sm" />
            <p className="mt-1 text-xs text-gray-500">This name appears when someone pays you via UPI.</p>
            <button onClick={handleSave} className="w-full bg-black text-white py-2.5 rounded-full text-sm">{saving ? "Saving..." : "Save changes"}</button>
            <button onClick={handleSignOut} className="w-full text-red-600 text-sm py-2">{signingOut ? "Signing out..." : "Sign out"}</button>
          </div>
        )}

        {!isOwnProfile && (
          <div className="rounded-2xl bg-white p-4 border border-black/5 space-y-2">
            <button onClick={() => setShowBlockDialog(true)} className="w-full rounded-xl border border-[#dbcab7] py-2 text-sm">{blockBusy ? "Please wait..." : "Block user"}</button>
            <button onClick={() => setShowReportDialog(true)} className="w-full rounded-xl border border-[#dbcab7] py-2 text-sm">Report user</button>
          </div>
        )}
        ...
        {!isOwnProfile && (user.instagram_url || user.gpay_link) && (
          <div className="flex gap-2">
            {user.instagram_url && (
              <a href={user.instagram_url} target="_blank" className="flex-1 text-center bg-pink-500 text-white py-2 rounded-xl text-xs">Instagram</a>
            )}
            {user.gpay_link && (
              <a href={generateUpiLink({ upiId: normalizeUpiId(user.gpay_link) }) || "#"} className="flex-1 text-center bg-green-600 text-white py-2 rounded-xl text-xs">
                Pay via UPI
              </a>
            )}
          </div>
      ...
      >
        <div className="space-y-2">
          <select value={reportReason} onChange={(e) => setReportReason(e.target.value)} className="w-full rounded-lg border border-[#d7c6b5] bg-white px-3 py-2 text-sm">
            {REPORT_REASONS.map((reason) => <option key={reason} value={reason}>{reason}</option>)}
          </select>
          <textarea value={reportDetails} onChange={(e) => setReportDetails(e.target.value)} rows={3} placeholder="Details (optional)" className="w-full rounded-lg border border-[#d7c6b5] bg-white px-3 py-2 text-sm" />
        </div>
      </ActionDialog>
    </div>
  );
